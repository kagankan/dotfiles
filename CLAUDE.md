...
- **このリポジトリは外部公開・複数マシンで共有する前提のため、以下の情報を含めない。** 作業時・コミット前に確認すること
  - 特定の組織・会社の名称やドメイン
  - 特定のリポジトリ名・プロジェクト名
  - 個人を特定できる情報（メールアドレス、アカウント ID 等）
  - 内部 URL・内部システムのホスト名
  - これらの情報が必要な設定はマシン固有ファイル（`CLAUDE.local.md`、`settings.machine.json` 等）に置く
